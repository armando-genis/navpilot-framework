...
V4l2CameraDevice::capture() {
  ...
  const bool is_mjpg =
    (cur_data_format_.pixelFormat == V4L2_PIX_FMT_MJPEG ||
    cur_data_format_.pixelFormat == V4L2_PIX_FMT_JPEG);

  // For RAW: copy the whole expected frame.
  // For MJPG: copy only the valid payload.
  const size_t copy_bytes = is_mjpg
...
if (cur_data_format_.pixelFormat == V4L2_PIX_FMT_YUYV) {
  ...
  img->step = cur_data_format_.bytesPerLine;   // valid
