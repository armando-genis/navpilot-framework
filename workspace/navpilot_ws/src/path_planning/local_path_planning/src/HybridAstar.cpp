...
HybridAstar::HybridAstar(Grid_map grid_map, CarData car_data, double simulationLength, double step_car, const vector<Eigen::VectorXd> &reference_path)
    : grid_map_(grid_map), car_data_(car_data), simulationLength(simulationLength), step_car(step_car), reference_path_(reference_path)
{
    cout << purple << " ----> Hybrid Astar Initialized <----" << reset << endl;

    // cout the size of the reference path
    cout << purple << "---> Reference path size: " << reference_path_.size() << reset << endl;
