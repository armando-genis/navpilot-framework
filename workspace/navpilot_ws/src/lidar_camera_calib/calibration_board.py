...
board_height_px = int(board_height_m * mm_per_m) # 800 px
...
circle_centers = [
    ...
    (center_x - offset_center_px, center_y + offset_center_px),  # (-0.15, +0.15)
    (center_x, center_y - offset_center_px),                     # (0, -0.15)
]
...
# Top-left corner is (0, 0), origin at top-left
centers_aruco = {}
...
print("Saved calibration board as calibration_board.png")

# ========= SAVE PDF VIA PILLOW =========
# Convert NumPy array (uint8, 2D) to Pillow image
img = Image.fromarray(board, mode="L")  # 'L' = 8-bit grayscale
img = img.convert("RGB")  # PDF likes RGB better

# Set DPI so that 1 px ~ 1 mm when printed at 100%
# 1200 px / 47.24 in ≈ 25.4 dpi  (since 1200 mm / 25.4 = 47.24 in)
dpi = 25.4
...
# Create SVG content
# Convert to millimeters for SVG dimensions (1 px = 1 mm)
