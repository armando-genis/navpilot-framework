...
MultiCameraSubscriber::MultiCameraSubscriber() : Node("multicamera_detection_tensort_node")
{
  ...
  // Detector placeholder: no YOLO-CPP; detections stay empty until another backend is plugged in
  RCLCPP_INFO(this->get_logger(), "Detection backend not loaded; poses will be empty.");
...
void MultiCameraSubscriber::inferenceLoop()
{
  while (true)
  {
    ...
    // ── Detections (none until another backend is plugged in); postprocessing unchanged ──
    {
      ...
      std::vector<PoseResult> poses;  // empty; replace with your detector output later
