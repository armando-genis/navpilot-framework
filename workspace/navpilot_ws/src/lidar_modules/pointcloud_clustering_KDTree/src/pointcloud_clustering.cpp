...
pointcloud_clustering_node::pointcloud_clustering_node(const rclcpp::NodeOptions& options) : Node("pointcloud_clustering_KDTree_node", options)
{
    // Parameters
    ...
    this->declare_parameter("USE_CONCAVE_HULL", true);
    ...
    // Create subscriber
    sub_points_cloud_ = this->create_subscription<sensor_msgs::msg::PointCloud2>("/velodyne_points_rotated_notground", 10, std::bind(&pointcloud_clustering_node::pointCloudCallback, this, std::placeholders::_1));
    ...
    RCLCPP_INFO(this->get_logger(), "\033[1;34m----> USE_CONCAVE_HULL: %d\033[0m", USE_CONCAVE_HULL);
    RCLCPP_INFO(this->get_logger(), "\033[1;34m----> HULL_ALPHA: %f\033[0m", HULL_ALPHA);
}
...
void pointcloud_clustering_node::pointCloudCallback(const sensor_msgs::msg::PointCloud2::ConstSharedPtr& msg)
{
...
void pointcloud_clustering_node::convex_hull(const std::vector<pcl::PointCloud<pcl::PointXYZ>::Ptr>& cloud_clusters)
{
    ...
    for (const auto& cluster : cloud_clusters)
    {
        ...
        // Clusters are already downsampled before clustering; use directly for hull
        if (USE_CONCAVE_HULL)
        {
        ...
        if (hullCloud->empty())
        {
            std::cout << red << "Hull is empty" << reset << std::endl;
            continue;
        }
        ...
        for (const auto& point : hullCloud->points)
        {
