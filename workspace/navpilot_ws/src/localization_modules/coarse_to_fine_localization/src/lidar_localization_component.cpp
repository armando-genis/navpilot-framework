...
: rclcpp_lifecycle::LifecycleNode("coarse_to_fine_localization", options),
  clock_(RCL_ROS_TIME),
  ...
  broadcaster_(this)
{
  ...
  declare_parameter("ndt_coarse_resolution", 2.0);
  declare_parameter("ndt_coarse_step_size", 0.3);
  declare_parameter("ndt_coarse_max_iterations", 20);
  declare_parameter("map_crop_half_xy", 50.0);
  declare_parameter("map_crop_half_z", 10.0);
...
void PCLLocalization::initializeParameters()
{
  ...
  get_parameter("ndt_coarse_resolution", ndt_coarse_resolution_);
  get_parameter("ndt_coarse_step_size", ndt_coarse_step_size_);
  get_parameter("ndt_coarse_max_iterations", ndt_coarse_max_iterations_);
  get_parameter("map_crop_half_xy", map_crop_half_xy_);
  get_parameter("map_crop_half_z", map_crop_half_z_);
...
void PCLLocalization::initializeRegistration()
{
  ...
  if (registration_method_ == "GICP") {
    ...
    registration_ = ndt;
    coarse_ndt_.reset(new pcl::NormalDistributionsTransform<pcl::PointXYZI, pcl::PointXYZI>());
    coarse_ndt_->setResolution(ndt_coarse_resolution_);
    coarse_ndt_->setStepSize(ndt_coarse_step_size_);
    coarse_ndt_->setMaximumIterations(ndt_coarse_max_iterations_);
    coarse_ndt_->setTransformationEpsilon(transform_epsilon_);
  }
    ...
    coarse_ndt_omp_ = pclomp::NormalDistributionsTransform<pcl::PointXYZI, pcl::PointXYZI>::Ptr(
      new pclomp::NormalDistributionsTransform<pcl::PointXYZI, pcl::PointXYZI>());
    coarse_ndt_omp_->setResolution(ndt_coarse_resolution_);
    coarse_ndt_omp_->setStepSize(ndt_coarse_step_size_);
    coarse_ndt_omp_->setMaximumIterations(ndt_coarse_max_iterations_);
    coarse_ndt_omp_->setTransformationEpsilon(transform_epsilon_);
    if (ndt_num_threads_ > 0) {
      coarse_ndt_omp_->setNumThreads(ndt_num_threads_);
    } else {
      coarse_ndt_omp_->setNumThreads(omp_get_max_threads());
    }
  }
...
void PCLLocalization::cloudReceived(const sensor_msgs::msg::PointCloud2::ConstSharedPtr msg)
{
  ...
  float pose_x = init_guess(0, 3);
  float pose_y = init_guess(1, 3);
  float pose_z = init_guess(2, 3);

  pcl::PointCloud<pcl::PointXYZI>::Ptr target_cloud(new pcl::PointCloud<pcl::PointXYZI>);
  if (full_map_cloud_ptr_ && full_map_cloud_ptr_->size() > 0 && map_crop_half_xy_ > 0 && map_crop_half_z_ > 0) {
    pcl::CropBox<pcl::PointXYZI> crop;
    crop.setMin(Eigen::Vector4f(
      pose_x - static_cast<float>(map_crop_half_xy_),
      pose_y - static_cast<float>(map_crop_half_xy_),
      pose_z - static_cast<float>(map_crop_half_z_),
      1.0f));
    crop.setMax(Eigen::Vector4f(
      ...
      pose_z + static_cast<float>(map_crop_half_z_),
      1.0f));
    crop.setInputCloud(full_map_cloud_ptr_);
    crop.filter(*target_cloud);
    if (target_cloud->size() == 0) {
      target_cloud = full_map_cloud_ptr_;
    }
  } else {
    target_cloud = full_map_cloud_ptr_;
  }

  pcl::PointCloud<pcl::PointXYZI>::Ptr registration_target;
  if (registration_method_ == "GICP" || registration_method_ == "GICP_OMP" ||
      registration_method_ == "NDT" || registration_method_ == "NDT_OMP") {
    pcl::PointCloud<pcl::PointXYZI>::Ptr filtered_target(new pcl::PointCloud<pcl::PointXYZI>());
    voxel_grid_filter_.setInputCloud(target_cloud);
    voxel_grid_filter_.filter(*filtered_target);
    registration_target = filtered_target;
  } else {
    registration_target = target_cloud;
  }
  ...
  Eigen::Matrix4f align_init = init_guess;
  if (registration_method_ == "NDT" && coarse_ndt_) {
    coarse_ndt_->setInputSource(tmp_ptr);
    coarse_ndt_->setInputTarget(registration_target);
    coarse_ndt_->align(*output_cloud, init_guess);
    if (coarse_ndt_->hasConverged()) {
      align_init = coarse_ndt_->getFinalTransformation();
    }
    registration_->align(*output_cloud, align_init);
  } else if (registration_method_ == "NDT_OMP" && coarse_ndt_omp_) {
    coarse_ndt_omp_->setInputSource(tmp_ptr);
    coarse_ndt_omp_->setInputTarget(registration_target);
    coarse_ndt_omp_->align(*output_cloud, init_guess);
    if (coarse_ndt_omp_->hasConverged()) {
      align_init = coarse_ndt_omp_->getFinalTransformation();
    }
    registration_->align(*output_cloud, align_init);
  } else {
  ...
  if (!enable_timer_publishing_){
    ...
    //Limitacion de cantidad de poses guardads
    const size_t MAX_PATH_SIZE = 1000;
      if (path_ptr_->poses.size() >= MAX_PATH_SIZE) {
        path_ptr_->poses.erase(path_ptr_->poses.begin());
      }
...
void PCLLocalization::timerPublishPose()
{
  ...
  //Mismo limite de cantidad holaaaa, soy fondon JAJDSA
  const size_t MAX_PATH_SIZE = 1000;
    if (path_ptr_->poses.size() >= MAX_PATH_SIZE) {
    path_ptr_->poses.erase(path_ptr_->poses.begin());
  }
